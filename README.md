...
| `execution_ruler.py` | **Objective ruler for ACTIVE roles** (code-writer/debugger): applies the model's PATCH to the buggy code, runs the gold's TRUSTED test, pass = the test now succeeds. Truth is EXECUTABLE, not opinion — so golds need no hand-authored "expected output". Trust-split (patch is the only untrusted input; argv-only, `shell=False`); `require_isolation=True` **fail-closed** (refuses without an injected OS-sandboxed `run_fn` — a tempdir+subprocess is a correctness fence, not a security boundary); oracle hard-guard (re-materializes tests AFTER apply; strips planted `conftest.py`/`*.pth`); **tolerant** whole-or-nothing applier (exact → rstrip → whitespace-insensitive, unique-match guard, keeps the file's real indentation); surfaces the test `stderr` in the verdict; NEVER raises. |
| `wsl_sandbox.py` | **The real OS-sandboxed `run_fn`** injected into `execution_ruler` (WSL2: `unshare --net --mount --pid`, tmpfs over `/mnt` so the host FS is hidden, ephemeral ext4 workspace, `isolation_capable=True`). Turns the ruler from fail-closed into executing. Honest limit: shares the WSL2 kernel — a strong fence, not an anti-hostile-tenant boundary (swap for nsjail/microVM without touching the ruler). |
...
| `harvest_validate.py` | **Validates harvested golds red→green** before trusting them: RED = parent tree + test fails; GREEN = fix tree passes; materializes the FULL parent tree via `git archive` (BugsInPy semantics, so sibling imports resolve). File-only fallback when the `::function` selector collects nothing. The gate that turns a raw fix-commit into a *measurable* gold. |
...
| `gold_preflight.py` | **Gold pre-flight (D6 author-error guard):** vets a case labeled *clean* before spending — kills the recurring failure where a human mislabels a trap as clean, which would silently deflate over-flag scores. Opt-in from `role_eval`. |
...
| `loop.py` | **The ACTIVE loop (auto-repair, verified by execution).** `orchestrate(task, *, budget_usd, max_iters, roster_writer, roster_debugger, run_fn, ...)` closes the loop over the fan-out: ROUTE (`run_role`) → INTEGRATE (tolerant applier, first candidate that applies) → VERIFY (`execution_ruler` + sandboxed `run_fn`) → DEBUG (feeds the test stderr back) → repeat, with four hard stops (success / budget / iterations / stuck). `budget_usd` is mandatory (no infinite default). The half that RESOLVES over the consultative subagents; the oracle is execution (the model never authors its own judge, D6/D7); real-world merge stays HUMAN (D5). Fully injectable → hermetic tests. |
